...
from hp.dict import MyOrderedDict as od
...
class Session( #main session handler. runs many simulations for stochastic modelling
    ...
    def load_models(self):
        ...
        self.fdmg = self.spawn_child(childname = 'fdmg', kid_class = fdmg.scripts.Fdmg)
    ...
    def load_timeline(self):
        ...
        # secondary
        #=======================================================================
        self.year0 = self.timeline_d.values()[0].year #take the year from the first time step object
    ...
    def run_session(self, wtf=None): #run each simulation
        ...
        for sim_key in sim_l:
            ...
            #else: logger.debug('sensi_f = FALSE')
    ...
    def write_fancy_res(self):
        ...
        for k, v in self.outs_od.iteritems():
...
class Simulation( #a single simulation within the session
    ...
    def run_sim(self): #run the full timeline
        ...
        logger.info('with %i Tsteps: %s'%(len(self.timeline_d), (self.timeline_d.keys())))

        #=======================================================================
        # loop and run
        #=======================================================================
        cnt = 0
        for time_str, tstep_o in self.timeline_d.iteritems():
...
class Tstep( #a timestep from teh simulation timeline
    ...
    def check_tstep(self):
        ...
        for k, v in self.run_seq_d.iteritems():
            
            if not k in self.session.models_d.keys():
                raise IOError
    ...
    def run_dt(self): #loop through the model dictionary and run each one
        ...
        logger.debug('running %i models: %s'%(len(d), d.keys()))
        #=======================================================================
        # execute each assigned model
        #=======================================================================
        cnt = 1
        for model_name, run_seq_l in d.iteritems(): #loop through 
...
class Action(    #collection of modifications of the objects in the Fdmg
    ...
    def setup_action(self, container=wdict): #special child raising per the mod_n_l
        ...
        try:
            self.dynp_wd = hp.dict.subset(self.session.dynp_d, self.dynp_n_l, 
                                         container = container, logger = logger)
        except:
            d = self.session.dynp_d
            bool_l = hp.basic.bool_list_in_list(d.keys(), self.dynp_n_l)
            
            if bool_l.sum() > 0:
                dynp_ar = np.array(self.dynp_n_l)
                dynp_lost_ar = dynp_ar[~bool_l]
                
                logger.error('some Action.dynp_n_l were not loaded on the \'dynp\' tab: %s'%dynp_lost_ar.tolist())
                
            else: 
                logger.error("dict subset failed for some reason")
            
            raise IOError
                
            
        
        if self.dynp_wd is None: self.dynp_wd = container()
        ...
        if not self.act_n_l is None:
            ...
            logger.debug('finished with %i sub actions collected: %s'%(len(self.acts_d), self.acts_d.keys()))
    ...
    def run(self, actk_d = None , container=wdict): #execute the chained Actions and all my dynp
        ...
        logger.debug('executing with\n    actk_d: %s \n    acts_d: %s \n    dynp_wd.keys(): %s'%
                     (k, self.acts_d, self.dynp_wd.keys()))
        ...
        if not self.acts_d is None:
            ...
            for act_n in self.act_n_l:
                ...
                logger.debug('on my pick (%i) running nested act_n: \'%s\' and their dynp_wd: %s \n '
                             %(len(actk_d1), act_n, act_o.dynp_wd.keys()))
        ...
        if len(self.dynp_wd) > 0:
            logger.debug('running my dynps: %s'%self.dynp_wd.keys())
            ...
            for dynp_n in self.dynp_n_l:
                ...
                logger.debug('running dynp \'%s\' with \'%s\' on my set (%i) of \'%s\' \n'
                             %(dynp_n, dynp_o.value_assign_str, len(actk_d1), actk_d1.values()[0].__class__.__name__))
                #===================================================================
        ...
        logger.info('changed %i objects with %i sub-Actions and %i prime-dynps: %s'
                    %(len(upd_all_s), sub_act_cnt, len(self.dynp_wd.keys()),self.dynp_wd.keys()))
    ...
    def mark_set(self, d): #add an entry to the parents childmeta_df marking that this Action has been applied
        ...
        # setup
        #=======================================================================
        obj1 = d.values()[0] #just take the first
