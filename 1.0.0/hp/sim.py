...
class Sim_wrap(object): #worker for simulation handing
    ...
    def __init__(self, 
        ...
        logger.debug("finishe __init__ on \'%s\' as  \'%s\' \n"%(self.name, self.__class__.__name__))
        return
    ...
    def get_dt_n(self, kwd): #intelligent timeline setting
        ...
        # setup
        #=======================================================================
        if not isinstance(kwd, basestring): raise IOError
    ...
    def reset_simo(self): #reset attribute values to those found in teh reset_d
        ...
        #=======================================================================
        logger.debug('on %i attributes in reset_d: %s'%(len(self.reset_d), self.reset_d.keys()))
        cnt = 0
        for attn, attv in self.reset_d.iteritems():
            #===================================================================
            # print out
            #===================================================================
            if self.db_f:
        ...
        if len(self.reset_func_od) > 0:
            ...
            for func, caller in self.reset_func_od.iteritems():
...
class Sim_session(Sim_wrap):#, hp.outs.Out_controller):#, hp.dynp.Dynp_controller): #wrapper for Session instances3
    """
    special object for the top level session handler for a simulation objects
    """    
    #===========================================================================
    # program pars
    #===========================================================================
    ...
    def get_sim_metadf(self, sim_cnt = None): #build a dummy frame for the sims
        ...
        df = pd.DataFrame(index = range(0, int(sim_cnt), 1),
    ...
    def reset_all(self): #reset all objects in the family to their base state
        ...
        logger.debug('on %i books in the family library: %s'%(len(self.family_d), self.family_d.keys()))
