...
class Data_wrapper(object): #wrapper for data type operations
    ...
    def __init__(self, *vars, **kwargs):
        ...
        #self.logger.debug('Data_wrapper terminated the init cascade')
    ...
    def loadr_real(self,  #load data for filepaths
        ...
        if not os.path.exists(filepath):
            logger.error('passed filepath does not exist: \n    %s'%filepath)
            raise IOError  
        ...
        if filepath.endswith('.csv'):  
            ...
            logger.error('got unexpected filetype (%s) from filepath: \n    %s'%(filepath[:-4], filepath))
            raise IOError
        ...
        if self.session._write_ins: _ = hp.basic.copy_file(filepath, self.session.inscopy_path) #copy py files to outputs
    ...
    def harmonize_data(self,  #harmonize/combine two datasets to 1
        ...
        if datonames_list is None: gid_l = self.kids_d.keys() #merge everything
        if wtf is None: wtf = self.session._write_data
    ...
    def all_uq_child_slices(self, search_head, value_head): #make a sliced child for each unique entry in the search_head
        ...
        logger.debug('finished slicing %i children: %s'%(len(new_kids_d), new_kids_d.keys()))
        ...
        if hasattr(self, 'headpath') & hasattr(self, 'tailpath'): #concanate to find file
            if not self.headpath is None:
                ...
                #relative
                    ...
                    if self.headpath.startswith(u'\\'): raise IOError
