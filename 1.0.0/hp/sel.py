...
class Sel_usr_wrap(object): #functions for objects that want to use Selectors
    ...
    def __init__(self, *vars, **kwargs):
        ...
        if not self.sel_n is None: 
            try: 
                ...
                if not self.sel_n in self.session.sels_d.keys():
                    logger.error('passed selector \'%s\' not found in set'%self.sel_n)
                raise IOError
            
        #=======================================================================
        # set handles for this pclass_n
        #=======================================================================
        k1 = self.session.family_d[self.pclass_n].values()[0] #just get the first kid
    ...
    def sel_usr_chk(self):
        ...
        if not self.pclass_n in self.session.family_d.keys(): raise IOError #this object needs to be loaded
    ...
    def make_pick(self, #makes the selection from selector,  owner logic, and passed set
        ...
        if not len(pick_d)> 0:  
            ...
            logger.debug('made pick_d %i of \'%s\''%(len(pick_d), pick_d.values()[0].__class__.__name__))
        ...
        if (not self.upd_sim_lvl == 'none'):
            ...
            self.pick_d = copy.copy(pick_d)
            self.pick_k_ar = np.array(pick_d.keys())
            #===================================================================
            # reseting
            #===================================================================
    ...
    def check_pick(self, pick_d):
        ...
        #=======================================================================
        first = True
        for key, kid in pick_d.iteritems():
    ...
    def slice_handler(self, #find the interescection of a passed d with your own pick (accounts for hierarchy)
        ...
        bool_ar = np.isin(self.pick_k_ar, big_d.keys())
        ...
        if not np.any(bool_ar): #no matches
            ...
            #get a new boolean
            bool_ar = np.isin(self.pick_k_ar, vert_d.keys())
        ...
        if self.db_f:
            if not np.all(np.isin(self.pick_k_ar, d_og.keys())): raise IOError
    ...
    def reset_selusr(self): 
        ...
        if not self.picko_p_f:
            ...
            if not self.pick_d is None:
                ...
                #=======================================================================
                if self.db_f:
                    if 'pick_d' in self.reset_d.keys(): 
...
class Selector( Sel_usr_wrap,
    ...
    def load_data(self): #load the data passed for this Selector on the 'selectors' tab
        ...
        if self.db_f: self.check_data(data)
    ...
    def calc_selection(self, big_d = None, container=wdict): #perform the selection calculation based on the pars passed in the 'selectors' tab
        ...
        logger.debug('with class_d \'%s\': %i : %s'%(self.pclass_n, len(class_d), class_d.keys()))
        ...
        if not self.metadf_bool_exe is None:
            ...
            pparent_o = class_d.values()[0].parent
        ...
        #=======================================================================
            ...
            for gid, obj in class_d.iteritems(): #loop through each object and ccheck
                #check if this fits the boolean
                try:
                    if eval(exe_str): pick_d[gid]= obj
                except:
                    logger.error('failed to evaluate \'%s\''%exe_str)
                    raise IOError
        ...
        if self.db_f:
            ...
            if len(pick_d) == 0:
                ...
                k1 = pick_d.values()[0]
                if not getattr(k1, self.key_att) == pick_d.keys()[0]:
                    raise IOError
        ...
        self.pick_k_ar = np.array(pick_d.keys(), dtype=object)

        return pick_d

    
    def ranked_choice(self, #selection from some ranked list
                      n='udev', #number of entries to select from list
        ...
        if n == 'udev':
            ...
            n = self.session.udev.infil_cnt + self.session.udev.infil_cnt_delta
            'not really using the infil cnt delta, but this would allow for changes in pick size over time'
            logger.debug('n=udev. pulling from session.udev.infil_cnt %s'%n)
        ...
        if self.db_f:
            if n > self.ranked_l:
                logger.error('trying to pick more than we have in the list! (%i)'%len(self.ranked_l))
                raise IOError
        ...
        if self.model.bucket_size == 0:
            l = self.ranked_l[:n] #get a static copy of the ranked_l
            
            'shouldnt run out of picks...'

            
        elif self.model.bucket_size > 0:
            l = self.get_random_pick_from_bucket(self.ranked_l, n + self.model.bucket_size, n, stoch_f = stoch_f)
                        
        else: raise IOError #unexpected KWARG for Udev.pick_type
        ...
        #=======================================================================
        for index, obj_n in enumerate(l): #loop through the ranking
            ...
            if obj is None:
                ...
                logger.error('on run_cnt %i (loop=%i, n=%i) could not find \'%s\' in class_d (%i): %s'
                               %(self.run_cnt, index,n, obj_n, len(class_d), class_d.keys()))
            ...
            pick_d[obj.gid] = obj
            #update the list
            if update: self.ranked_l.remove(obj_n) #take this one out of the list
        ...
        if self.db_f:
            ...
            pick_nl = []
            for gid, obj in pick_d.iteritems():
            ...
            if update:
                ...
                if np.any(np.isin(pick_nl, self.ranked_l)):
                    bool_l = np.isin(pick_d.keys(), self.ranked_l)
    ...
    def write_pick(self, 
        ...
        with open(self.pick_file, 'a') as f:
            ...
            #custom attribute
                ...
                'allows writing name or CPID'
                for gid, obj in self.pick_d.iteritems():
...
class Sel_controller(object): #control functions for Selsector objects
    ...
    def run_selectors(self): #run teh recalc routine on all the selectors if flagged as so
        ...
        for sel_n, sel_o in self.session.sels_d.iteritems():
