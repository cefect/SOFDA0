...
def view_df(df, logger=mod_logger): #run DF viewer
    raise IOError #doesnt seem to be working
    import dfgui
    
    
    'todo: add handlign for mdex'
    
    if not isdf(df): 
        logger.error('not a df')
        raise IOError
    elif not smart_isdx(df):
        #remap the columns as strings
        newcols = []
        for entry in df.columns: newcols.append(str(entry))
        df.columns = newcols

        dfgui.show(df)
    else:

        lvl0_vals = df.columns.get_level_values(0)
        dfnew = df[lvl0_vals[0]]
        
        dfgui.show(dfnew)
    
    logger.info('opened df using dfgui. close window to continue')
...
def v2(df):
    ...
    webbrowser.open(df.style.render())
...
def slice_df_from_file(df_raw, slicefile_path, #slice a df with values found in an external slicefile
    ...
    for header, col in slicefile_df2_clean.iteritems(): #loop through each column and update the logical df
...
def load_xls_df(filepath, logger=mod_logger, 
    ...
    if not isinstance(filepath, basestring): raise IOError
    'todo: add some convenience methods to append xls and try again'
    if not os.path.exists(filepath): 
        logger.error('passed filepath not found: %s'%filepath)
        raise IOError
    ...
    try:
        ...
        logger.error('unable to read xls from: \n    %s'%filepath)
        raise IOError
...
def load_xls_d(filepath, #load a xls collection of tabs to spreadsheet
    ...
    logger.debug('loaded %i dfs from xls sheets: %s'%(len(df_d.keys()), df_d.keys()))
...
def force_dtype_from_tplate(df_raw, templatefile_path, logger=mod_logger): #force the df column dtype to match that provided in the template
    ...
    for header, col in df_raw.iteritems(): #loop through each column
...
def df_to_1line_dict(df_raw, convert_none=True, logger=mod_logger): #convert the first two rows of a df into a dictionary
    ...
    for header, col in df_raw.iteritems():
    ...
    dictionary = dict(zip(keys_list, values_list)) #zip these into a dcit
...
def dict_to_ser(key_ser, dictionary): #generates a series from the dictionary keys
    ...
    for index, entry in key_ser.iteritems():
...
def ser_fill_df(ser, rows, logger = mod_logger): #fill a dataframe with this series
    ...
    df = pd.DataFrame(index = range(0, rows), columns = ser.index.values)
...
def right_fill_df_dict(right_dict, df_raw, logger=mod_logger): #add a dummy block of attribute values to the df_raw
    ...
    logger.debug('added %i new columns to make df_merge %s: %s'
                 %(len(right_dict), str(df_merge.shape), right_dict.keys()))
...
def write_dfset_excel(df_set_dict, filepath, #write a dictionary of frames to excel
    ...
    for df_name, df in df_set_dict.iteritems():
    ...
    writer.save()
    logger.info('wrote %i frames/tab to:  %s'%(len(df_set_dict.keys()), filepath))
...
def write_fly_df(filepath, data, #write the first row of the df 
...
def get_conflicts_bool(df_raw, header='CPID', logger=mod_logger):
...
def log_full_df(df, logger=mod_logger): #override display defaults and log the full frame
    ...
    with pd.option_context('display.max_rows', None, 
                           ...
                           'display.height',1000,
                           'display.width',1000):
                           #'expand_frame_repr', True): #discontinued
...
def fillna_fancy(left_df_raw, fill_df_raw, #fill the holes in the left_df_raw with the right
    ...
    for example:
        a set of partial assessment records for each CPID
...
def merge_rnk_choice(left_df_raw, right_df_raw, ranking_col, ranked_l, #fill the left with the right using the rnk_d to rank entries
    ...
    for example:
        a skinny data set on CPIDs field responses
...
def clean_datapars(df_raw,  #typical formatting for cleaning a datapars df
    ...
    boolcol = df4.columns == 'name'
    if boolcol.sum() > 0:
        try: df4['name'] = df4['name'].astype(np.string_)
        except:
            raise IOError
        
        if np.any(df4.duplicated(subset = 'name')):
            logger.error('got some duplicates on the names column: %s'%df4.loc[df4.duplicated(subset = 'name'), 'name'].values.tolist())
            logger.debug('%s'%df4['name'].values.tolist())
            raise IOError
       
    """looks like we try and drop these columns a lot... 
    this replacess the null
    #path columns
    boolcol = df4.columns.str.contains('path')
    'not really working because weve already changed to floats'
    if boolcol.sum() > 0:
        df5 = df4.copy()
        
        df5.loc[:,boolcol] = df4.loc[:,boolcol].astype(np.string_)
    ...
    if flag:
        for entry in found: #only run check on those columns with the _f suffix
            if entry.endswith('_f'): 
                logger.error('make sure there are no empty cells in \'%s\''%entry)
                raise IOError
            """
            hp.pd.v(df5)
            """
...
def clean_kill_flag(df_raw, #clean the data frame with kill_flags
    ...
    if len(df2) == 0: 
        ...
        logger.warning('got empty frame. stop flag in wrong spot?')
        #time.sleep(1)
        """allow this
        raise IOError"""
    #===========================================================================
...
def get_incremental(ser_raw, logger=mod_logger): #get the incremental (delta) values 
    ...
    for index, cum_val in ser_raw.iteritems(): #loop through and claculate
...
def Get_mdex_xs_lvl_rank_dict(mdex, lvl_values, lvl_names, logger=mod_logger): #build a dictionary of the passed indexer xs
    ...
    lvl_rank_dict = dict(zip(lvl_ranks, lvl_values))
...
def Get_spatial_dxcol(x_values,y_values,time_ser, data_raw, logger=mod_logger):  #build a dxcol of x,y,z over time data
    ...
    tup = zip(lvl0_values, lvl1_values)
...
def dxcol_to_df_set(dxcol, 
    ...
    df_set_dict = container(zip(dfname_list, df_list))
