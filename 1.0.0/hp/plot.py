...
def setup_video(logger = mod_logger, basedir = u'G:/My Drive/Programs/ffmpeg/bin'): #setup the ffmpeg codec
...
class Plotdatabundler(object): #convenience methdos for gneerating plotdatabundles
    ...
    def get_pdb_dict(self,  #take a list of datos and get a dictionary of pdbs
        ...
        if isinstance(dato_list, dict):
            dato_dict = dato_list
            dato_list = []
            for name, [datoname, dato, data_ar] in dato_dict.iteritems():
        ...
        logger.debug('bundled %i entries into an od: %s'%(len(pdb_dict), pdb_dict.keys()))
    ...
    def get_equil_slice_bundle(self, dep_ar_dict): #return a bundle sliced as in the dep_ar_dict
        ...
        # loop and fill
        #=======================================================================
        for index,[dep_name, dep_dato, dep_ar] in dep_ar_dict.iteritems(): #loop through each and build the plot
    ...
    def inspect_pdb(self, pdb): #send pdb to logger
        ...
        if isinstance(pdb, dict):
            ...
            for index,[dep_name, dep_dato, dep_ar] in pdb.iteritems(): #loop through each and build the plot 
    ...
    def sort_buckets(self, kids_d, in_search_str): #sort children into buckets witha search string
        ...
        for gid, obj in kids_d.iteritems():
...
class Plot_o(Plotdatabundler): #worker for plotting teh data
    ...
    def plot_twinx(self, dep_pdb_d_left, dep_pdb_d_right, #plot multiple on twinned axis
        ...
        if ax1 == None:
            ...
            #=======================================================================
            #left axis
            indp1_dato1 = indp1_pdb_dict.values()[0][1]
            dep1_dato1 =  dep_pdb_d_left.values()[0][1]
            dep2_dato1 =  dep_pdb_d_right.values()[0][1]
    ...
    def plot_bundles(self, dep_pdb_dict, indp_pdb_dict=None, #plot multiple dep_dataos against teh same indp data
        ...
        #=======================================================================
        #get dummy datos
        indp_dato1 = indp_pdb_dict.values()[0][1]
        dep_dato1 =  dep_pdb_dict.values()[0][1]
        ...
        first = True
        for index,[dep_name, dep_dato, dep_ar] in dep_pdb_dict.iteritems(): #loop through each and build the plot
