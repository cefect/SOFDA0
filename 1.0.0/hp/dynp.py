...
# # IMPORTS --------------------------------------------------------------------
#===============================================================================
import os, sys, copy, random, logging, weakref, time
#re
...
class Dynamic_par(hp.sel.Sel_usr_wrap,
    ...
    def calibrate_valf(self): #calibrate value function generator
        ...
        if hp.basic.isnum(vstr): #numerics
        ...
        elif isinstance(vstr, basestring): #everything else (String type)
            ...
            # string calls
            #===================================================================
            elif isinstance(vstr_py, basestring): #still a string
    ...
    def calibrate_scipy(self, vstr):
        ...
        #binary label based
        elif isinstance(self.min, basestring):
    ...
    def valf_simpl(self, obj): #allow simple math on the object
        logger = self.logger.getChild('valf_simpl')
        """
        obj.model.time.strftime('%Y')
        """
        ...
        try:
            ...
            logger.error('failed to exec \'%s\' on \'%s\''%(exe_str, obj.name))
            raise IOError
            
        """
        new_val = max(old_val, obj.fhzlvl_ser[100])
        """
        if new_val is None: 
            logger.error('failed to set any new_val for %s'%exe_str)
            raise IOError
        elif hp.basic.isnum(new_val):
            new_val = float(new_val)
        
        logger.debug('with exec \'%s\' on \'%s\' got %s = \'%s\' (vs %s)'
                     %(exe_str, obj.name, att_name, new_val, old_val))
    ...
    def valf_binary(self, obj):  #binary label based
        ...
        if self.db_f:
            ...
            self.hist_df.loc[obj.gid, 'CPID'] = obj.CPID
    ...
    def pick_from_dict(self, d): #select an attribute value from a liklihood dictionary
        ...
        cuml = 0
        for k, v in d.iteritems(): #loop through each option and see if the roll falls within the band
    ...
    def apply_to_set(self,  #set teh sample value on all the objects in your class_D
        ...
        if self.db_f:
            if not callable(self.val_f): 
                logger.error('something is wrong with the value setting function')
                raise IOError
            
            #class check
            obj1 = dynk_d.values()[0]
        ...
        logger.debug('on dynk_d (%i) of \'%s\' as \'%s\'\n'
                     %(len(dynk_d), dynk_d.values()[0].__class__.__name__, self.value_assign_str))
        cnt = 0
        for k, obj in dynk_d.iteritems():
            ...
            if self.freeze_att_f:
                ...
                if self.db_f:
                    if obj.is_frozen(self.att_name): 
                        logger.error('change attempted on frozen \'%s.%s\''%(obj.gid, self.att_name))
                        raise IOError #check it
...
class Dynp_controller(object): #controlling dynamic pars
    ...
    def run_dynps(self): #set apply all applicable dynp_os for this upd_sim_lvl
        ...
        #=======================================================================
        for dynp_n, dynp_o in od.iteritems():
            ...
            logger.debug('running \'%s\' with upd_sim_lvl = %i\n'%(dynp_n, dynp_o.upd_sim_lvl))
            #===================================================================
            # run the dynp
            #===================================================================
...
class Dynp_session(Dynp_controller): #handling of th edynamic pars
    ...
    _dynp_hnd_coln_l = ['att_name', 'self_upd', 'dynk_hndl', 'upd_df', 'lock_f']
    ...
    def load_dynp_hnd_file(self, filepath): #load the xls full of dynp handles  
        ...
        if not self.__class__.__name__ == 'Session': raise IOError
        if filepath is None: raise IOError
        #=======================================================================
        # load the file
        #=======================================================================
        ...
        #checks
        #=======================================================================
        logger.debug('looping through %i loaded object handles: %s'%(len(hnd_d), hnd_d.keys()))
        for k, df_raw in hnd_d.iteritems():
            ...
            #===================================================================
            try:
                df = df_raw.drop(columns = ['desc'])
            except:
                logger.error('unable to drop the desc column')
            
            #header check
            bool_l = hp.basic.bool_list_in_list(df.columns.tolist(), self._dynp_hnd_coln_l)
            if not np.all(bool_l): raise IOError #check that all the expected were found
            
            if np.any(pd.isnull(df)): 
                logger.error('found some nulls on \'%s\' in teh dynp file: %s'%(k, filepath))
                raise IOError #check there are no nulls

            """better to keep this explicitly as none
            #reset all 'none's to null
            boolar = df == 'none'
            df1 = df[~boolar]
            
            logger.debug('reset %i none values to nan'%boolar.sum().sum()) """
            
            """ moved all this onto the dyno
            self.set_parent_complexity(k, df1)
            
            #check that the objects have all the methods passed
            self.check_dynh(k, df1)
    ...
    def build_dyno_pars_d(self): #build the parameters shared by each dyno
        ...
        for cn, df in self.dynp_hnd_d.iteritems():
            d = dict() #pars holder for this cn
            
            d['hndl_df'] = df
            
            d['dyn_anl'] = df.loc[:,'att_name'].values.tolist()
            
            
            # #find locked attribute names
            boolidx = df.loc[:,'lock_f'] #should already be a booledan
            d['lock_anl']= df.loc[boolidx, 'att_name'].values.tolist()
            
            #get secondary command parameters
            d['dynk_hndl_d'], d['dynk_ns'] = self.build_dynk_hndl(df)
            
            logger.debug('loaded dyno pars for \'%s\': %s'%(cn, d.keys()))
            
            dyno_pars_d[cn] = copy.copy(d) #set this
            
        logger.debug('finished loading dyno pars on %i dyno classes:%s'%(len(dyno_pars_d), dyno_pars_d.keys()))
    ...
    def build_dynk_hndl(self, df): #build handles from the handle frame
        ...
        for ind, row in df.iterrows():
            ...
            d = hp.basic.str_to_dict(row['dynk_hndl'])
            
            dynk_ns.update(d.keys()) #udpate the dynk_cn set
    ...
    def get_sensi_metadf(self, sims_per_var = None, wtf = None): #get the metadata for sensitivity analysis runs
        ...
        #=======================================================================
        baseline_ser = pd.Series(index = self.dynp_d.keys()) #row to add each time
        
        logger.debug('finding baseline values for %i dynps'%len(self.dynp_d))
        
        for dn, dynp in self.dynp_d.iteritems(): #loop through each dynp and enter the value
        ...
        for dn, dynp in self.dynp_d.iteritems(): #loop through by classname
