...
class Outputr(hp.sel.Sel_usr_wrap,
        ...
        if self.otype == 'simple':
            ...
            if self.time_d == 0:
                ...
                if self.space_d == 0:    #one value for the simulation
                    ...
                    data = pd.Series(name = self.codename, index = pick_d.keys(), dtype = np.object).sort_index()
    ...
    def run_outp(self): #store outputs for each of my userse
        ...
        # loop and store
        #=======================================================================
        for gid, obj in d.iteritems(): #loop through all of your users and store the data page
    ...
    def get_stat(self, data=None): #get the passed stats for this data set
            ...
            #matrix type data
                ...
                if len(data) == 0:
                    ...
                    try:  
                        ...
                        logger.error('failed to evaluate \'%s\''%(self.sim_stats_exe))
                        raise IOError
        ...
        #=======================================================================
        if self.db_f:
            if not isinstance(exe_str, basestring):
                raise IOError
...
class Out_controller(object): #thin wrapper of output commands for high level simulation controllers
    ...
    def check_sensi_delta_outrs(self):
        ...
        for oname in l:
            if not oname in self.outs_od.keys():
    ...
    def run_outputrs(self, outs_od=None): #tell each writter to store its  data to the library
        ...
        #=======================================================================\
        cnt = 0
        for oname, outputr in outs_od.iteritems():
            #===================================================================
    ...
    def get_outputs_summary(self, od = None): #calculate the stats across the timeline for each output
        ...
        # loop and calc stats
        #=======================================================================
        for name, outo in od.iteritems(): #loop through each page and calculate stats
            #===================================================================
            # drop to single stat
            #===================================================================
    ...
    def write_full_outputs(self,  #write the attributes to file
        ...
        for tag, outo in outs_od.iteritems(): 
            if outo.data is None: continue #skip this one
            d[tag] = outo.data
            
        #=======================================================================
        # add the extras
        #=======================================================================
        for tag, data in xtra_d.iteritems():
            d[tag] = data
    ...
    def plot_hists(self, wtf=None): #plot hisograms on the flagged outputr codenames
        ...
        ptype = []
        for codename, outputr in outs_od.iteritems():
