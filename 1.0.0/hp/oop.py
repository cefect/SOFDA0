...
# IMPORTS ----------------------------------------------------------------------
import logging
import os, sys, copy, gc, time, weakref, psutil, inspect
...
def attach_att_df(obj, df, 
    ...
    for index, row in df.iterrows(): #loop through each entry and attach it to the class object
        #=======================================================================
        # defaults
        #=======================================================================
        var_name = row[0]
        ...
        if isinstance(value_raw, basestring):
            #filepath
            if value_raw[1] == ':': #see if its a file path
                value = str(value_raw)
                
            #generic conversion (checks for lists, tuples, etc)    
            else: 
                value = hp.basic.excel_str_to_py(value_raw)"""
        ...
        if db_f:
            try:
                ...
                if not isinstance(value, type(oldval)):
                    logger.warning('for \'%s\' got type mismatch from old \'%s\' to new \'%s\''%(var_name, type(oldval), type(value)))
            except:
                pass
...
def log_all_attributes(obj, log_callable = False, log_def = False, logger=mod_logger): #log each attribute
    ...
    for att_name, att_value in att_std.iteritems():
        logger.debug('\'%s\' = \'%s\' (std)'%(att_name, att_value))
    
    if log_callable:
        for att_name, att_value in att_call.iteritems():
            logger.debug('\'%s\' = \'%s\' (callable)'%(att_name, att_value))
            
    if log_def:
        for att_name, att_value in att_def.iteritems():
...
def copy_container_d(d, logger = mod_logger): #a semi deep copy of a container of objects
    ...
    logger.debug('on d with %i entries: %s'%(len(d), d.keys()))
...
def deepcopy_objs(d, container = dict, logger=mod_logger):  #semi deep copy of object
    ...
    for key, obj in d.iteritems():
        ...
        #=======================================================================
        #logger.debug('setting copies of %i attributes'%(len(obj.__dict__)))
        for n, _ in obj.__dict__.iteritems():
            v = getattr(obj, n)
            try:
                nv = copy.deepcopy(v)
            except:
                #logger.debug("failed to make a deep copy of att \'%s\' of type \'%s\'. using shallow"%(n, type(v)))
                nv = v
                'setting copies seems to duplicate the objects'
                
            #set this
            setattr(obj_copy, n, nv)
...
def mirror_att( #attach an attribute (by name) from the parent onto the child (with some type logic)
                child, att_n, 
                parent=None, 
                match_self_type_f = True, #match the type of the attribute as it is now
                db_f = None,
                logger=mod_logger):
    #===========================================================================
    # defaults
    ...
    if match_self_type_f:
        ...
        new_v = hp.basic.match_type(old_v, new_v, logger=logger, db_f=db_f)
...
class Kid_condenser(): #collect all children in the tree matchin gthe pick_cn 
    ...
    def __init__(self, d, pick_cn, 
        ...
        #=======================================================================
        d = d[SOME LABEL] = obj
            generally pulled out of the family_d
        
        """
        
        self.logger = logger.getChild('Kid_condenser')
    ...
    def drop_all(self): #start the drop cascade
        ...
        logger.debug('condensing d (%i) of \'%s\' into \'%s\' with: \n %s'
                     %(len(self.d), self.d.values()[0].__class__.__name__, self.pick_cn, self.d.keys()))
    ...
    def drop(self,d): #callabel for each level of the hierarchy
        ...
        logger.debug("drop_loop_cnt = %i with d (%i) of \'%s\'"
                     %( self.drop_loop_cnt, len(d),d.values()[0].__class__.__name__))
        ...
        for _, obj in d.iteritems():
            ...
            if cn == self.pick_cn:
                ...
                if len(obj.kids_d) > 0:
                    ...
                    if self.db_f:
                        if cn == 'Dfunc': 
                            if obj.dfunc_type == 'dfeats': raise IOError
...
class Child(hp.prof_mem.Profile_wrapper):#foundational object 
    ...
    def __init__(self,
        ...
        logger.debug('start __init__ as \"%s\''%name)
        ...
        if not att_d is None:
            ...
            logger.debug('overwriting __dict__ from att_d (%i): %s'%(len(att_d), att_d.keys()))
        ...
        # standard inherits
        logger.debug('inherit_logr() from \'%s\' to \'%s\' \n'%(parent.name, self.name))
        ...
        logger.debug('oop.Child initilzied as \'%s \' with class \'%s\' with %i atts\n'
                          %(self.name, self.__class__.__name__, len(self.__dict__)))
        """
        #=======================================================================
        # WARNING
        #=======================================================================
    ...
    def init_clone(self, parent, shadow=False): #commands for the birthing of a clone (or original)
        """
        ...
        #logger.debug('set gid \"%s\' now runnin gupd_family_d() \n'%self.gid)
        ...
        if not self.session.state == 'init':
            if hasattr(self, 'halt_update'): self.halt_update()
    ...
    def set_debugger_mode(self): #load debugger flag from pars file
        ...
        logger = mod_logger.getChild('set_debugger_mode') #have to use this as our own logger hasnt loaded yet
        #=======================================================================
        ...
        try:
            ...
            try: 
                boolidx = df.loc[:, 'name'] == self.__class__.__name__
                if boolidx.sum() == 1:
                    self.db_f = bool(df.loc[boolidx, 'db_f'].values) #pull yourself out
                else: raise IOError
                
            except: 
                logger.warning('failed to load \'%s\' from the obj_test pars'%self.__class__.__name__)
                raise IOError
        except: 
            logger.warning('failed to get the obj_test pars')
            #raise IOError
    ...
    def set_name(self, #set the name logic
        ...
        if self.db_f:
            ...
            if not isinstance(name, basestring): raise IOError
    ...
    def inherit_from_pset(self, parent=None, inher_set = None):
        ...
        # update ht elist
        #=======================================================================
        if inher_set is None: 
            s = self.inherit_parent_ans
        ...
        #===================================================================
        's is combined with spc_s by __init__'
        logger.debug('on %i entries in the s: %s'%(len(s), list(s)))
        ...
        for attn in s:
            ...
            if self.db_f:
                if hasattr(self, attn):
                    ...
                    logger.warning('I dont have an attribute \'%s\''%attn)
                    oldval = None
            ...
            try:
                ...
                setattr(self, attn, v)
                #logger.debug('set \'%s\' as \'%s\''%(attn, type(v)))
            except:
    ...
    def attach_att_ser(self,  #attach the passed series as attributes
        ...
        # precheck
        #=======================================================================
        """ moved this to raise_chidlren_df
        if att_ser_skip_anl is None: 
            att_ser_skip_anl = self.parent.att_ser_skip_anl
        #=======================================================================
        # cleaning
        #=======================================================================
        boolidx = ~att_ser_raw.index.isin(att_ser_skip_anl) #slice out the exclusion
        att_ser = att_ser_raw[boolidx].dropna()"""
        ...
        # loop and apply
        #=======================================================================
        for att_name, att_value in att_ser.iteritems():
            ...
            try:
                ...
                #logger.debug('attaching \'%s\' as \'%s\''%(att_name, type(att_value)))
            except:
                logger.error('got unexpected type on for \'%s\' =  %s'%(att_name, type(att_value)))
                raise IOError
            
            #logger.debug('attached \'%s\' with value = %s'%(att_name, att_value))
        ...
        # wrap up
        #=======================================================================
        logger.debug('attached %i attributes to \'%s\''%(len(att_ser), self.name))
        return
    ...
    def write_named_atts(self, #extract and write these attributes from teh child (one file per attribute)
        ...
        logger.debug('bundled and wrote %i attributes'%len(att_d.keys()))
    ...
    #===========================================================================
    # def data_writer(self, data = None, filepath = None, filename = None): #generic file writer
    #     #=======================================================================
    #     # defaults
    #     #=======================================================================
    #     logger = self.logger.getChild('data_writer')
    #     
    #     #=======================================================================
    #     # path steup
    #     #=======================================================================
    #     if filepath is None:
    #         if filename is None: filename = self.name +' data.csv' 
    #         filepath = os.path.join(self.outpath,filename )
    #     else: 
    #         if not filename is None: raise IOError
    #         
    #     #=======================================================================
    #     # root folder setup
    #     #=======================================================================
    ...
    #     #=======================================================================
    #     if hp.pd.isdf(data):
    #         hp.pd.write_to_file(filepath, data, logger = logger)
    #     else: #TODO: add some more data types
    #         raise IOError
    #===========================================================================
...
class Parent(object): #wrapper for raising child
    ...
    def update_childmeta(self, obj, method = 'add'): #add the passed object to your metadata
        ...
        #=======================================================================
        if self.db_f:
            if not obj.name in self.kids_d.keys(): 
                raise IOError
            """
        ...
        if method == 'add':
            ...
            for ind, entry in ser.iteritems():
                #get from teh child
                v = getattr(obj, ind)
    ...
    def raise_children(self): #raise the children intelligently
        ...
        #check if you are in the tree map
            ...
            if child_blvl in self.treem_df_d.keys():
        ...
        if self.db_f: 
            logger.debug('finished with %i new kids: %s'%(len(new_kids_dict), new_kids_dict.keys()))
    ...
    def raise_gkids_group(self, kid_colname, childmeta_df, #makes a slice of the meta_df and raises achild with that group
        ...
        logger.debug('finished with %i twogen_gkids_d: %s'%(len(twogen_gkids_d), twogen_gkids_d.keys()))
    ...
    def raise_children_df(self, #raise all the children from teh metadf and put them in the kids_d
        ...
        if self.db_f:
            ...
            if not 'name' in df2.columns.values: 
                logger.error('passed df does not have a \'name\' column: %s'%df2.columns.values)
                raise IOError
        ...
        if self.db_f:
            stop = time.time()
            logger.info('finish %.4f s (%.4f/obj) built %i from df:\n %s'
                         %(stop - start, float((stop-start) / float(cnt)), len(new_kids_d), new_kids_d.keys()))
            #logger.debug('\n') 
    ...
    def spawn_child(self,  #create a child from teh passed data
        ...
        child = kid_class(self, self.session, 
                          name = childname, 
                          att_d = att_d, 
                          att_ser = att_ser,
                          shadow = shadow, 
    ...
    def adopt_child(self, child): #activates the child under this parent (and all post_init commands)
    ...
    def set_child_atts_df(self, #attach a df by key to each child
        ...
        # setup
        #=======================================================================
        #find where teh CPID column is for slicing later
        ...
        # loop and update
        #=======================================================================
        for oname, obj in kids_d.iteritems(): #loop through each hosue and apply the new atts
    ...
    def get_kids_byname(self, search_name): #find the children in the kids_d with names matching
        ...
        found_d = dict()
        for childname, dato in self.kids_d.iteritems():
    ...
    def swap_kids(self, shdw_kids_d): #replace current kids with a shadow set
        ...
        if self.db_f:
            ...
            """I think this is holding onto some kids
            #kid type check
            old_k1 = self.kids_d.values()[0]
            new_k1 = shdw_kids_d.values()[0]
            
            if not old_k1.__class__.__name__ == new_k1.__class__.__name__:
                raise IOError
            
            if old_k1.perm_f:
                raise IOError #only allwoed to swap non permanent objects
            
            
            if self.kids_d == shdw_kids_d:
                raise IOError #duplicate detected """
        ...
        if self.db_f:
            ...
            nk_ar = np.array(self.kids_d.keys())
            sk_ar = np.array(shdw_kids_d.keys())
    ...
    def activate_kids(self, new_kids_d): #bring a set of shadow children into the family structure
        ...
        logger.debug('on new_kids_d %i: %s'%(len(new_kids_d), new_kids_d.keys()))
                
        #=======================================================================
        # bring the new kids in
        #=======================================================================
        for key, child in new_kids_d.iteritems():
    ...
    def update_kids_d(self, obj, method='add'):
        ...
        if method == 'add':
            ...
            if self.db_f:
                if obj.name in self.kids_d.keys(): raise IOError
    ...
    def kill_kids(self): #erase your kids_d
        ...
        d = copy.copy(self.kids_d)
        
        if not self.kids_d is None:
            logger.debug('killing %i kids and resetting kids_d and kids_sd'%len(d))
            
            #delete the object and its reference in the family _lib
            for k, v in d.iteritems():  self.session.kill_obj(v)
    ...
    def check_family(self, d = None, d_is_all = True): #check each object in passed for consistency against your children
        logger = mod_logger.getChild('check_kids')
        #=======================================================================
        # defaults
        #=======================================================================
        if d is None: 
        ...
        for k, v in d.iteritems():
            ...
            if not v.name in self.kids_d.keys(): raise IOError
            if not v.name in self.childmeta_df.loc[:,'name'].tolist(): raise IOError
            if not v.dfloc in self.childmeta_df.index.tolist(): raise IOError
    ...
    def update_kids_sd(self, obj, method='add'): #add the object to your structured kids_d
        ...
        if method == 'add':
            ...
            logger.debug('adding \'%s\' as \'%s\' to kids_sd with %i books: %s'
                         %(obj.name, cn, len(self.kids_sd), self.kids_sd.keys()))
    
            if cn in self.kids_sd.keys(): #append
            
                d_old = self.kids_sd[cn]
                
                if obj.name in d_old.keys(): 
...
#===============================================================================
    def load_par_tabs(self, #load data from a spreadsheet of parameters
        ...
        for sheetname, df_raw in df_d_raw.iteritems(): #looop through each sheet/frame
            if sheetname.startswith('~'): continue #skip these
            
            logger = logger.getChild(sheetname) #so we can tell what sheet the subfunctions are working on 
        ...
        #attacha nd closeout
        logger.debug("pars_df_d %i dfs from files: %s"%(len(pars_df_d), pars_df_d.keys()))
...
class Session_o(Trunk_o,#generic class for a program session
    ...
    def __init__(self, 
        ...
        'redundant?'
        self.session        = self #this helps the raise_children get started
    ...
    def upd_family_d(self, obj, shadow=False):
        ...
        if cn in d.keys(): #see if you have a book already
            #family dictionary
        ...
        if self.db_f:
            ...
            if obj.gid in class_d.keys(): 
                logger.error('there is already a copy in book \'%s\' of \'%s\''%(cn, obj.gid))
                raise IOError
    ...
    def parent_swap(self, obj, parent_new): #swap parents
        ...
        #=======================================================================
        if self.db_f:
            if obj.name in parent_old.kids_d.keys(): raise IOError
            if not obj.name in obj.parent.kids_d.keys(): raise IOError
    ...
    def family_cnt(self): #get the total count of objects in the family_d
        ...
        cnt = 0
        for cn, book in self.family_d.iteritems(): cnt += len(book)
        
        return cnt
    
    def print_family_cnt(self):
        logger = self.logger.getChild('print_family_cnt')
        for cn, book in self.family_d.iteritems():
            logger.info('%i in \'%s\''%(len(book), cn))
    
    
    def kill_obj(self, proxy): #kill the passed object
        ...
        #=======================================================================
        # retrieve the object from teh family library
        #=======================================================================
        gid = proxy.gid
        ...
        del self.family_d[cn][gid] #delete the reference
    ...
    def check_family_refs(self,#check the reference count of all objects
        ...
        # loop and check
        #=======================================================================
        logger.debug("checking %i books for max_cnt %i: %s \n"%( len(self.family_d),max_cnt, self.family_d.keys()))
        for bookname, book_wd in self.family_d.iteritems():
            ...
            for k, obj in book_wd.iteritems():
