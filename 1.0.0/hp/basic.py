...
def copy_file(filetocopy_path,  #copy file to a directory
              dest_base_dir, overwrite = True, logger=mod_logger): 
    """
    #===========================================================================
    # INPUTS
    #===========================================================================
    ...
    dest_file_path = os.path.join(dest_base_dir, filename)
...
def setup_workdir(workpath, basename='out', ins_basename = '_inscopy', logger=mod_logger): #Create and return output directory based on time
    ...
    if not os.path.exists(workpath): 
        print 'passed workpath does not exists: \'%s\''%workpath
        raise IOError #working path not found
    ...
    logger.info('outpath built: %s'%outpath)
...
#===============================================================================
# def get_fname_matches( #get a set of fpaths that contain the search_str within the dir
#                     search_dir, #directory to search in 
#                     search_str, #string to search filenames for
#                     
#                     logger=mod_logger,
#                     **re_kwargs):
#     
...
#     # defaults
#     #===========================================================================
#     logger = logger.getChild('get_fname_matches')
#     
#     fnd_s = OrderedSet()
#     
#     #===========================================================================
#     # #scan this directory and build a set of matching filenames
#     #===========================================================================
#     for dirpath, dirnames, filenames  in os.walk(search_dir):
#         logger.debug('searching %s'%dirpath)
#         #loop through the files in this folder
#         for fname in filenames:
#             if re.search( search_str,fname, **re_kwargs):
#                 fpath = os.path.join(dirpath, fname)
#                 fnd_s.update([fpath]) #add this path
#                 
#                 logger.info('found match: %s'%fname)
#                 
#     logger.debug('found %i matches in directory \'%s\''%(len(fnd_s), search_dir))
#     
#     return list(fnd_s)
#===============================================================================
...
def is_str_list(chk_str, list_pfx = '[', logger = mod_logger): #check if  a strin gmay be a list
    ...
    if not isinstance(chk_str, basestring): raise IOError
...
def str_is_none(raw_str, logger=mod_logger): #detect whether teh passed string is probably noen
    ...
    if not isinstance(raw_str, basestring): return False
...
def str_to_dict(raw_str, container = dict, logger=mod_logger):
    ...
    if not str_is_dict(raw_str): raise IOError
...
def excel_str_to_py(raw_str, logger = mod_logger): #convert user input excel strings to pythonic
    if not isinstance(raw_str, basestring): return raw_str #short cut out
...
def match_type(old_obj, new_raw,  #attempt to return a new_obj matching the type of the old object
               logger=mod_logger, db_f = False): 
    
    if old_obj is None: return new_raw
    if new_raw is None: return new_raw
    
    if db_f: logger = logger.getChild('match_type')
    ...
    if isinstance(old_obj, float): 
        ...
        try:
            ...
            logger.warning('failed to map int onto \'%s\'... keeping as %s'%(new_raw, type(new_raw)))
            return new_raw

    elif isinstance(old_obj, basestring): 
        new_obj = str(new_raw)
    ...
    if db_f:
        if not isinstance(new_obj, type(old_obj)):
            if not isinstance(old_obj, basestring):
