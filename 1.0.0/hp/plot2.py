...
class Pair( #pair of variables
        ...
        for attn in ['var1_nm', 'var2_nm', 'var3_nm']:
            ...
            if not v is None:
                try:
                    ...
                    if not v in all_vars_d.keys():
                        logger.error('could not find \'%s\' in the scenario variables'%v )
                    raise IOError
        

            
        logger.debug('finished with %i pairs collected: %s'%(len(d), d.keys()))
    ...
    def trim_to_union(self): #trim vars to common index
        ...
        for name, varo in self.vars_d.iteritems():
    ...
    def set_atts_from_var(self):
        ...
        #=======================================================================
        first = True
        for k, v in self.vars_d.iteritems():
            if first:
...
class Plotr( #collection of pairs for plotting
    ...
    def __init__(self, *vars, **kwargs):
        ...
        if isinstance(self.figsize, basestring):
    ...
    def check_plotr(self):
        ...
        for k, v in self.pairs_d.iteritems():
            if v.name in nm_l:
    ...
    def basic(self,
        ...
        if ax is None:
            ...
            pair1 = pairs_d.values()[0] #using for default formatting
            
            if not pair1.flip_f:
                xo1 = pair1.vars_d.values()[1]
                yo1 = pair1.vars_d.values()[0]
            else: #flipped axis
                xo1 = pair1.vars_d.values()[0]
                yo1 = pair1.vars_d.values()[1]
        ...
        logger.debug('adding %i pairs to plot: %s'%(len(pairs_d), pairs_d.keys()))
        for pn, pair in pairs_d.iteritems():
            ...
            #===================================================================
            if not pair.flip_f:
                xvar = pair.vars_d.values()[1]
                yvar = pair.vars_d.values()[0]
            else:
                xvar = pair.vars_d.values()[0]
                yvar = pair.vars_d.values()[1]
        ...
        for pn, (func, vars, annkwargs) in annos_d.iteritems():
    ...
    def callouts(self, #scatter plot with callouts
        ...
        # add the labels
        #=======================================================================
        for pnm, pair in pairs_d.iteritems():
    ...
    def hist(self, #scatter plot with callouts
        ...
        if ax is None:
            ...
            #axis labels
            if self.xlab is None:
                xlab = pair1.vars_d.values()[0].label #pull from teh first variable of the pair
            else:
                xlab = self.xlab
    ...
    def box(self, #generate a box plot
        ...
        pos_l = range(0, len(pairs_d), 1) #get the set of positions
        ...
        if ax is None:
            ...
            #y label
            if self.ylab is None:
                ylab = pair1.vars_d.values()[0].label #pull from teh first variable of the pair
            else:
                ylab = self.ylab
...
class Plot_handler(object): #wrapper for complex data objects to handle plotting
    ...
    def run_all_plots(self):
        ...
        for nm, plotr in self.plotrs_d.iteritems():
            plotr.plot()
